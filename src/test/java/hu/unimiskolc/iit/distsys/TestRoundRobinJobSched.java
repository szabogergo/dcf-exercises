...
public class TestRoundRobinJobSched {
	...
	public void testRRWithPMFiller() throws Exception {
		...
		Assert.assertEquals("Should have all PMs running as all should be occupied with VMs", myIaaS.machines.size(), myIaaS.runningMachines.size());
		...
		Assert.assertEquals("Should have exactly the specified number of VMs on the IaaS", requestedVMcount, vms.size());
